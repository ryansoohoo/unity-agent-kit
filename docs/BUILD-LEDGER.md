...
- COMPLETENESS, because the bullets above read like a cleaner win than the artifacts support: unity-verify's OWN-SET positives are FLAT, 16/30 baseline -> 16/30 final. The whole +4 bucket gain lives in verify-expect queries sitting in OTHER eval sets (11/18 -> 15/18), and +3 of that is one query — "after the refresh, how do i confirm the new type is attachable" in the unity-recipes set, 0/3-with-3-cross-fires -> 3/3.
- Against that, four general-verification queries regressed by 6 records between baseline and final: "rider says no errors but unity still runs the old behaviour" 3 -> 1, "please can you confirm the new inventory code…" 3 -> 1, "before I hand this branch off, prove the C# still typechecks" 3 -> 2, "verify the refactor didn't break anything before I push" 1 -> 0.
- So the SHIPPED description's TRAIN is below baseline: 27/33 vs 28/33 (all-expect, excluding the premise-gap query). This is the same trade ver1 was rejected for, smaller — a specific surface bought at some cost to the general one. The adoption stands on the held-out rule (16/24 >= 15/24), on the mechanism (three attachable-phrased queries unanimously 0/3 -> 3/3 across two sets) and on the cross-fire elimination. It does not stand on a clean sweep, and the record has to say so.
